...
from .data_models import DataType, StudyInfo, VolumeData
...
class VolumeLoaderService:
    ...
    def load_volume_from_study(self, study_info: StudyInfo) -> VolumeData:
        ...
        self.logger.debug(
            f"Loading volume: StudyUID={study_info.study_uid}, SeriesUID={study_info.series_uid}"
        )
        ...
        try:
            ...
            self.logger.debug(
                f"Volume loaded in {loading_time:.2f}s: shape={volume_data.volume.shape}"
            )
    ...
    def _load_dicom_volume(self, study_info: StudyInfo) -> VolumeData:
        ...
        if "file_list" in study_info.metadata:
            raw_file_list = study_info.metadata["file_list"]
            if not isinstance(raw_file_list, list):
                raise TypeError("DICOM metadata file_list must be a list")
            file_list = [Path(str(item)) for item in raw_file_list]
            self.logger.debug("Using provided file_list with %d files", len(file_list))
        ...
        volume, metadata = robust_load_dicom_volume(
            dicom_dir=dicom_dir, file_list=file_list, logger=self.logger
        )

        # Извлечение spacing и других метаданных
        raw_spacing = metadata.get("spacing", (1.0, 1.0, 1.0))
        raw_origin = metadata.get("origin", (0.0, 0.0, 0.0))
        if not isinstance(raw_spacing, (list, tuple)) or len(raw_spacing) != 3:
            raise ValueError(f"Invalid DICOM spacing: {raw_spacing!r}")
        if not isinstance(raw_origin, (list, tuple)) or len(raw_origin) != 3:
            raise ValueError(f"Invalid DICOM origin: {raw_origin!r}")
        spacing = (
            float(raw_spacing[0]),
            float(raw_spacing[1]),
            float(raw_spacing[2]),
        ...
        direction = np.asarray(
            metadata.get("direction", np.eye(3).ravel()),
            dtype=np.float64,
        ).reshape(3, 3)
        ...
        if any(s <= 0 for s in spacing):
            raise ValueError(
                f"Invalid spacing detected: {spacing}. All spacing values must be positive."
            )
    ...
    def _validate_dicom_uids_consistency(
        self, study_info: StudyInfo, file_list: list[Path] | None = None
    ) -> None:
        """
        ...
        if file_list is None:
            ...
            all_files = [dicom_dir / f if not f.is_absolute() else f for f in file_list]
    ...
    def _load_nifti_volume(self, study_info: StudyInfo) -> VolumeData:
        ...
        if any(s <= 0 for s in spacing):
            raise ValueError(
                f"Invalid spacing in NIfTI header: {spacing}. All spacing values must be positive."
            )
